...
def Calc_Returns(symbol, stock_info, start_date, end_date, logger):
    ...
    final_pnl_value = stock_info['Running_PnL', symbol].iloc[-1] # Retrieve last element
    starting_closing_price = stock_info['close', symbol].iloc[0] # retrieve initial investment
    start_pnl_value = stock_info['close', symbol].iloc[0]
